/*
 * 階梯 9：完整版 —— 這份跟 drivers/uart/pl011.c 完全相同。
 *
 * 誰會跑我： make step9
 *
 * 前八階問完之後，這裡是答案：init 為了 M9、volatile 為了 -O2、
 * TXFF 等待為了真硬體的 16 深 FIFO、CR/LF 轉換在 uart_putc 這一層。
 *
 * ---- 以下與 repo 版本逐字相同 ----
 *
...
/* 階梯 9 專用：讓它印一句話證明整條路是通的。 */
void kernel_main(void);
void kernel_main(void)
{
    uart_init();
    uart_puts("step9: same code as drivers/uart/pl011.c\n");
    while (1) { }
}
